...
| [`test_profile_2d.py`](test_profile_2d.py) | [`test_profile_2d.pyi`](test_profile_2d.pyi) | `widget`, `build`, `test_above_is_plus_y_in_2d`, `test_thin_containment_survives_tolerance_in_2d`, `test_planar_overlap_still_caught_in_2d` | The 2D profile: the plane is XY viewed from +Z, so "above" means +Y and topology ignores the degenerate Z (C1). |
...
| [`test_svg_parse.py`](test_svg_parse.py) | [`test_svg_parse.pyi`](test_svg_parse.pyi) | `test_emitter_output_round_trips_to_the_same_positions`, `test_plain_svg_uses_the_metre_contract_with_y_flip`, `test_prose_and_fences_around_the_svg_are_tolerated`, `test_a_completion_with_no_svg_is_a_value_error` | SVG round-trip: the emitter's output parses back to the same positions, and plain metre-unit SVG parses too. |
